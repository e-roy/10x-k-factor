# 10x K Factor — PWA Build Playbook (Drizzle + Postgres + Postgres rollups)

> **Scope**: Single, self‑contained execution guide for a **PWA‑first** implementation. Stack: **Next.js (App Router) + TypeScript + Tailwind + shadcn/ui + Drizzle ORM + Postgres (Neon/Supabase) + Upstash Redis + next‑pwa/Workbox + Web Push (VAPID) + MCP agents (Fastify/Node)** with **Postgres‑only analytics** (materialized views + cron). Mobile apps deferred.
...
**Assumptions:** Using **Auth.js (NextAuth)** with **Drizzle Adapter** on **Neon Postgres**. Provider: **GitHub OAuth** for simplicity (swap later as needed).
...
3. **NextAuth config**
   ...
   - Configure `providers: [GitHubProvider({ clientId, clientSecret })]`
...
5. **Login UI**
   - Landing page (`apps/web/app/page.tsx`): add a **Login** button: `onClick={() => signIn('github', { callbackUrl: '/app' })}`.
...
- For local/dev on Windows, GitHub OAuth avoids SMTP setup; add others later.

---

## 3) Phase 2 — Event Schema & Ingest (Postgres‑only)
...
- For each loop: trigger→share→open→join→FVM metrics present in Postgres rollups dashboards.
...
- Guardrail compute in Postgres rollups Pipe (complaints, opt‑outs, latency to FVM, fraud %).
...
## 16) Phase 15 — Dashboards (Postgres‑backed)

**Goal**: Operator view for K, funnels, cohort curves, and costs directly from **Postgres**.

**Tasks**

- Ensure MV `mv_loop_daily` exists (from Phase 2) and is refreshed via Vercel Cron every 5 min.
- Create additional MVs or SQL views as needed:
  - `mv_funnel_open_join_fvm` (per day, per loop)
  - `mv_retention_d1_d7` (simple cohort retention)
  - `mv_cost_per_rewarded_fvm` (join rewards + events)

- Build admin UI `/admin/metrics` using server actions that query these MVs. Add filters: date range, persona, subject, loop.
- Add CSV export endpoint that streams the query results.

**Acceptance Tests**

- K displays for selected date range/loop with no API timeouts.
- Changing filters re‑queries server actions and updates charts instantly.
- MV refresh endpoint logs success at least every 5 minutes.
...
REDIS_URL=...
TINYBIRD_TOKEN=...
TB_EVENTS_ENDPOINT=[https://api.tinybird.co/v0/events?name=events_raw](https://api.tinybird.co/v0/events?name=events_raw)
SMARTLINK_HMAC_SECRET=supersecret
VAPID_PUBLIC_KEY=...
VAPID_PRIVATE_KEY=...
NEXT_PUBLIC_APP_URL=[https://your.app](https://your.app)
...
- 4 loops fully wired; 4 agentic actions; presence + leaderboards; smart links + attribution; Postgres rollups dashboards (K, funnels); compliance memo; one‑take demo.
